...
8) Добавлена страница статистики пользователя (график по истории запросов + агрегированные таблицы + интерактивная карта пожарных рисков)
9) Подключена интеграция NASA FIRMS и расчёт итогового риска:
- В WeatherSearch добавлены поля lat/lon, risk_score и агрегаты FIRMS (firms_count, firms_avg_confidence).
- Итоговый риск 0..100 рассчитывается как комбинация погодного риска и активности термоточек FIRMS в радиусе ~50км за последние 1 сутки.
- На странице статистики риск отображается на интерактивной карте.